...
- 支持通知群配置
- 支持监控好友白名单
- 支持同房提醒、同房情况查询与可加入性提示
- 支持每日摘要日报、热门世界统计与推荐世界图文推送
...
| `coroom_notify_interval_seconds` | 同房提醒最短推送间隔秒数。同房提醒使用独立节流窗口，不与普通事件去重共用。 | `600` |
| `coroom_notify_min_members` | 同房提醒最少人数阈值。自动同房提醒与`/vrc同房情况`均按该阈值筛选（最小为2）。 | `2` |
| `coroom_notify_joinable_only` | 同房提醒是否仅推送“可加入”实例。仅影响自动提醒，不影响普通好友事件；`/vrc同房情况`仍显示并标注可加入性。 | `false` |
| `enable_daily_report` | 是否开启每日摘要日报。开启后会在设定时间向通知群发送当日摘要、热门世界和推荐世界。 | `false` |
| `daily_task_time` | 每日任务默认发送时间，格式 `HH:MM`。适用于所有“每天只发一次”的任务；若某任务存在独立时间配置，则优先使用独立配置。 | `21:00` |
| `daily_report_time` | 每日摘要日报发送时间，格式 `HH:MM`，按本地时间解释。 | `21:00` |
| `daily_report_top_n` | 日报中活跃好友/热门世界的展示数量。 | `5` |
...
| `/vrc监控列表` | 查看当前监控好友列表 |
| `/vrc同房情况` | 查看当前监控名单中的同房情况（受同房人数阈值影响） |
| `/vrc热门世界 [N]` | 查看今日“当天上线过好友”的热门世界 Top N |
| `/vrc生成日报 [推送]` | 生成今日摘要日报预览；带“推送”参数时推送到通知群 |
...
开启日报后，插件会在配置时间向通知群发送一份摘要，内容包括：

- 今日监控好友事件概览（上线 / 下线 / 状态变化 / 切图 / 同房）
- 今日活跃好友 Top N
- 今日热门世界 Top N
- 今日推荐世界（附图片）

如果推荐世界简介不是中文，会调用 AstrBot AI 机器人能力翻译为中文；若调用失败，则保留原文并标注“原文”。

### 热门世界统计

`/vrc热门世界` 与日报中的热门世界统计，均基于【当天上线过好友】（来自当日事件与当日在线快照）进行聚合，不会扫描全量历史数据。

### 世界简介翻译

当推荐世界的简介明显不是中文时，插件会调用 AstrBot AI 机器人能力进行翻译，并将结果缓存到本地数据库，避免对同一段简介重复翻译。若调用失败，则回退为原文并标注“原文”。
...
5. /vrc搜索好友 关键词
6. /vrc添加监控序号 1
7. 可选：执行 /vrc同房情况 或 /vrc热门世界 查看聚合视图
8. 可选：开启日报并在设定时间接收摘要，或手动执行 /vrc生成日报
```
...
- 新增配置项 `world_translation_cache_max_entries`（默认 `500`）。
- 在写入 `world_desc_translations` 后，按 `updated_at DESC` 仅保留最近 N 条，超出自动删除。
- 清理异常只记日志，不影响主流程。
